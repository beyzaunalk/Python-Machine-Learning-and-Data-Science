...
print(f"Yes,missing data, company clumn missing: {hotels['company'].isna().sum()} rows")
...
hotels['sum']=hotels['stays_in_week_nights']+hotels['stays_in_weekend_nights']
round(hotels['sum'].mean(),2)
...
hotels['total_paid'] = hotels['adr'] * hotels['sum']
...
hotels[hotels['total_of_special_requests'] == 5][['name','email']]# CODE HERE
...
round(((hotels['is_repeated_guest'] == 1).sum() / len(hotels))*100,2)
...
hotels['kids'] = hotels['babies'] + hotels['children']
hotels.sort_values('kids',ascending=False)[['name','adults','kids','babies','children']][:3]
